...
def quizzes():
    ...
    while quiz == "1" or "2" or "3": #Using one while loop for the menu
        if quiz == "1":
            ...
            leaderboard()
            #calling my leaderboard function whenever the user wants to leave the game
            sys.exit()
        #exitting the system if the user doesnt want play
...
def math():
    #opening and using my text file for my math function
    f=open("quizfile")
    lines=f.readlines()
    global mincorrectanswer
    mincorrectanswer = 0
    print("Welcome to the math quiz! Hope you enjoy this quick 5 questions quiz!")
#adding the questions in using a seperate text file
    q1 = str(input(print (lines[1]))) #q1 = quesiton 1
...
def leaderboard():
#opening a text file
    #Adding my global variables to tell the user how many questions they got wrong
    global mincorrectanswer
    global sincorrectanswer
    global hincorrectanswer
    f = open("leaderboard", "a")
    #importing date and time so I can record it in my textfile
    import datetime
    today = str(datetime.date.today())
    #Printing the users name and when they took it into my text file
    f.write("\n\n")
    f.write(name)
    f.write(" took the test on: ")
    f.write(today)
    f.write("\n\n")
    f.write(name)
    f.write(" got a total of:")
    totalwrong = sincorrectanswer + mincorrectanswer + hincorrectanswer
    f.write(str(totalwrong))
    f.write(" questions wrong.")
    #adding up the number of questions the user got wrong in each section
    f.close()
